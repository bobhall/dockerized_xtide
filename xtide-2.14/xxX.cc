// $Id: xxX.cc 5749 2014-10-11 19:42:10Z flaterco $
...
const Widget xxX::connect (int &argc, char **argv) {
  ...
  #ifdef HAVE_XAW3DXFT
    Xaw3dXftEncoding = 8;        // Encoding is Latin-1, not UTF-8.
    Xaw3dXftMenuSpacing = 0;     // Get rid of extra vertical spacing in menus.
    Xaw3dXftMultiColumnMenu = 1; // Make Set Time dialog not suck.
    Xaw3dXftNoHilitReverse = 1;  // Required for Xaw3dXftMultiColumnMenu.

    // Tweaks recommended by J.-P.
    // Xaw3dXftButtonDashed = 1; // Make button mouse-over harder to see.
    Xaw3dXftSetInsensitiveTwist((char*)"#a00000");
    Xaw3dXftSetHilitColor((char*)"#332211");
  #endif
...
void xxX::connectPostSettingsHook (Widget rootWidget) {
  ...
  monoFontArgValue = (XtArgVal)
    ...
    if (!cfbl.isNull) {
      ...
      if (!(Xaw3dXftDefaultFont = getFont(defaultFontName)))
	Global::barf (Error::CANT_LOAD_FONT, defaultFontName);
      defaultFontStruct = Xaw3dXftDefaultFont;
    } else {
      // Coerce DEFAULTFONT out of Xaw3dXft.c.
      defaultFontStruct = Xaw3dXftGetFont (display, NULL);
    }
  #else
